...
fn main() {
    ...
    // let n_s: f64 = 100.0;
    // // let n_t: f64 = n_s.powi(2) / 10.0;
    // let n_t: f64 = 1000.0;
    // let calc_method_fd_implicit = CalcMethod::FiniteDifference {
    //     kind: tree::FiniteDifferenceType::Implicit,
    //     n_s: n_s as u32,
    //     n_t: n_t as u32,
    //     s_max: 50.0,
    //     s_min: 0.0
    // };
    // let euro_krl_fd_implicit = KRL::new(Rc::clone(&euro_tree), 1000, Box::new(payoff_function1));
    // println!("KRL (Finite Difference implicit) created with {} stock steps, {} time steps", n_s as u32, n_t as u32);
    // let euro_krl_fd_implicit_price = euro_krl_fd_implicit.price(&calc_method_fd_implicit);
    // println!("Option price: {}", euro_krl_fd_implicit_price);
    ...
    // CRR & KRL with 10000 time steps (Backward Induction)
    let amer_crr_bi = CRR::new(Rc::clone(&amer_tree), 10000, Box::new(payoff_function1));
    ...
    let amer_krl_bi = KRL::new(Rc::clone(&amer_tree), 10000, Box::new(payoff_function1));
    ...
    // // CRR & KRL with 10000 time steps (Combinatorial)
    // let amer_crr_comb = CRR::new(Rc::clone(&amer_tree), 10000, Box::new(payoff_function1));
    ...
    // let amer_krl_comb = KRL::new(Rc::clone(&amer_tree), 10000, Box::new(payoff_function1));
    ...
    let euro_krl_fd_implicit = KRL::new(Rc::clone(&amer_tree), 1000, Box::new(payoff_function1));
    println!("KRL (Finite Difference implicit) created with {} stock steps, {} time steps", n_s as u32, n_t as u32);
    let euro_krl_fd_implicit_price = euro_krl_fd_implicit.price(&calc_method_fd_implicit);
    println!("Option price: {}", euro_krl_fd_implicit_price);
}
